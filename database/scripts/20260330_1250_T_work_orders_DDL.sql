...
CREATE TABLE IF NOT EXISTS workshop.work_orders (
    ...
    CONSTRAINT fk_workshop_work_orders_appointment_id        FOREIGN KEY (appointment_id)       REFERENCES workshop.appointments(id),
    CONSTRAINT fk_workshop_work_orders_employee_id           FOREIGN KEY (employee_id)          REFERENCES hr.employees(id),
...
COMMENT ON COLUMN workshop.work_orders.appointment_id        IS 'The appointment this work order was created from. Multiple work orders can reference the same appointment.';
COMMENT ON COLUMN workshop.work_orders.employee_id           IS 'The mechanic/employee assigned to this work order.';
